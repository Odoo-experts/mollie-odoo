...
class PaymentAcquirer(models.Model):
    ...
    mollie_api_key_prod = fields.Char(
        "Mollie Live API key",
        size=40,
        required_if_provider="mollie",
        groups="base.group_user",
    )
    create_shipment_notification = fields.Boolean(
        string="Create shipment Mollie",
    ...
    method_ids = fields.One2many(
        "payment.acquirer.method", "acquirer_id", "Supported methods"
    )
        ...
        mollie_tx_values.update(
            {
                ...
                and values.get("partner_country").code
                or "",
    ...
    def update_payment_icon_ids(self):
        ...
        for method in self.method_ids:
            ...
            icon.write(
                {"sequence": method.sequence, "provider": self.provider}
            )
        ...
        for acquirer in self:
            ...
            try:
                ...
                if methods.get("_embedded", False):
                    i = 10
                    for method in methods.get("_embedded", {"methods": []})[
                        "methods"
                    ]:
